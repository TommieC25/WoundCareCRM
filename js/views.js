...
const emailLinkH=phys?.email?` <a href="mailto:${phys.email}" onclick="event.stopPropagation()" style="color:#0a4d3c;font-size:0.75rem;">✉️ Email</a>`:'';const _locIdH=e.practice_location_id||(physicianAssignments[e.provider_id]?.find(a=>a.is_primary)||physicianAssignments[e.provider_id]?.[0])?.practice_location_id;const _locH=_locIdH?practiceLocations.find(l=>l.id===_locIdH):null;const phoneLinkH=_locH?.phone?` <a href="tel:${_locH.phone.replace(/\D/g,'')}" onclick="event.stopPropagation()" style="color:#0a4d3c;font-size:0.75rem;">📞 ${fmtPhone(_locH.phone)}</a>`:'';
const delFn=e.provider_id?`deleteNoteFromActivity('${e.id}','${e.provider_id}').then(()=>renderHistoryView())`:e.practice_location_id?`deletePracticeNote('${e.id}').then(()=>renderHistoryView())`:''
...
'<div class="contact-entries">'+filtered.map(e=>{const phys=e.provider_id?physMap[e.provider_id]:null;const physName=phys?fmtName(phys):(e.practice_location_id?getLocationLabel(e.practice_location_id):'Note');const locCtx=e.practice_location_id?getLocationContext(e.practice_location_id):'';const{time,text}=parseNoteTime(e.notes);const preview=text.length>120?text.slice(0,120)+'...':text;const emailLink=phys?.email?` <a href="mailto:${phys.email}" onclick="event.stopPropagation()" style="color:#0a4d3c;font-size:0.75rem;">✉️ Email</a>`:'';const _locIdA=e.practice_location_id||(physicianAssignments[e.provider_id]?.find(a=>a.is_primary)||physicianAssignments[e.provider_id]?.[0])?.practice_location_id;const _locA=_locIdA?practiceLocations.find(l=>l.id===_locIdA):null;const phoneLink=_locA?.phone?` <a href="tel:${_locA.phone.replace(/\D/g,'')}" onclick="event.stopPropagation()" style="color:#0a4d3c;font-size:0.75rem;">📞 ${fmtPhone(_locA.phone)}</a>`:'';const editFn=e.provider_id?`editNoteFromActivity('${e.id}','${e.provider_id}')`:e.practice_location_id?`editPracticeNote('${e.id}')`:''
...
overdue.forEach(r=>{
const phys=r.provider_id?physMap[r.provider_id]:null;const physName=phys?fmtName(phys):(r.practice_location_id?getLocationLabel(r.practice_location_id):'General Reminder');
...
const _locIdOD=r.practice_location_id||(physicianAssignments[r.provider_id]?.find(a=>a.is_primary)||physicianAssignments[r.provider_id]?.[0])?.practice_location_id;const _locOD=_locIdOD?practiceLocations.find(l=>l.id===_locIdOD):null;const phoneLink=_locOD?.phone?` <a href="tel:${_locOD.phone.replace(/\D/g,'')}" onclick="event.stopPropagation()" style="color:#0a4d3c;font-size:0.75rem;">📞 ${fmtPhone(_locOD.phone)}</a>`:'';
...
const phys=r.provider_id?physMap[r.provider_id]:null;const physName=phys?fmtName(phys):(r.practice_location_id?getLocationLabel(r.practice_location_id):'General Reminder');
const emailLink=phys?.email?` <a href="mailto:${phys.email}" onclick="event.stopPropagation()" style="color:#0a4d3c;font-size:0.75rem;">✉️ Email</a>`:'';
const _locIdT=r.practice_location_id||(physicianAssignments[r.provider_id]?.find(a=>a.is_primary)||physicianAssignments[r.provider_id]?.[0])?.practice_location_id;const _locT=_locIdT?practiceLocations.find(l=>l.id===_locIdT):null;const phoneLink=_locT?.phone?` <a href="tel:${_locT.phone.replace(/\D/g,'')}" onclick="event.stopPropagation()" style="color:#0a4d3c;font-size:0.75rem;">📞 ${fmtPhone(_locT.phone)}</a>`:'';
const {displayNotes,taskNote}=parseTaskRecord(r.notes);
const preview=displayNotes.length>120?displayNotes.substring(0,120)+'...':displayNotes;
const editFn=`editTaskFromList('${r.id}')`
const delFn=r.provider_id?`deleteNoteFromActivity('${r.id}','${r.provider_id}').then(()=>renderTasksView())`:r.practice_location_id?`deletePracticeNote('${r.id}').then(()=>renderTasksView())`:''
html+=`<div class="contact-entry" id="tlog-${r.id}" style="border-left:4px solid #f59e0b;background:#fffbeb;display:flex;gap:0.5rem;align-items:flex-start;cursor:pointer;" onclick="openTaskDetailModal('${r.id}')"><button onclick="event.stopPropagation();completeReminderInPlace('${r.id}','tlog-${r.id}',()=>{})" title="Mark complete" style="background:none;border:2px solid #f59e0b;color:#92400e;border-radius:50%;width:24px;height:24px;min-width:24px;cursor:pointer;font-size:0.8rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:0.15rem;">✓</button><div style="flex:1;"><div style="font-weight:600;color:#0a4d3c;">${physName}${phoneLink}${emailLink}</div>${taskNote?`<div style="font-size:0.85rem;font-weight:600;color:#92400e;background:#fef3c7;padding:0.2rem 0.5rem;border-radius:4px;margin-top:0.25rem;">📋 ${taskNote}</div>`:''}<div style="font-size:0.85rem;color:#333;margin-top:0.2rem;">${preview}</div><div style="font-size:0.7rem;color:#999;margin-top:0.2rem;">Note from ${r.contact_date}${r.author?' by '+r.author:''}</div>${editFn||delFn?`<div style="display:flex;gap:0.5rem;margin-top:0.4rem;">${editFn?`<button class="icon-btn" onclick="event.stopPropagation();${editFn}" title="Edit note & reminder date" style="font-size:0.85rem;">✏️ Edit</button>`:''}${delFn?`<button class="icon-btn" onclick="event.stopPropagation();${delFn}" title="Delete" style="font-size:0.85rem;color:#dc2626;">🗑️ Delete</button>`:''}</div>`:''}</div></div>`;
});
html+='</div></div>';
});
}
